...
AFRAME.registerComponent('constrained-look-controls', {
  ...
  schema: {
    ...
    limitLeft: {type: 'number', default: -90},
    limitRight: {type: 'number', default: 90},
...
constrainPanning: function (y) {
    var limit = 0;

    if (y > 0) {
      limit = this.data.limitLeft;
    } else if (y < 0) {
      limit = this.data.limitRight;
    }
    if (Math.abs(y) > Math.abs(limit)) {
      return limit;
    }
